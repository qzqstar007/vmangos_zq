...
#define __DAMAGE_MAX_SPELL  (350000)
	if (this->IsPlayer() && pVictim->IsCreature())
	{
        ...
        if(player->M_Achiv_Account_VIP & VIP_SPECIAL_SPELL_HASTE)
        {
            //VIP
            if (player->M_Leech_Spell) this->CastCustomSpell(this, ZQ_SPELL_LEECH_SPELL, (damage * player->M_Leech_Spell + 10) / 100.0f, 0, 0, true);
        }
        else 
        {
